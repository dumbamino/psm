...
import '../profile/profilescreen.dart'; // for AppColors
...
class _NewRecordScreenState extends State<NewRecordScreen> {
  ...
  Future<void> _initializeScreen() async {
    ...
    if (_currentUser == null) {
      if (mounted) {
        _showSnackbar('User session not found. Please log in again.',
            isError: true);
  ...
  void _loadInitialData(Map<String, dynamic> data) {
    _deceasedNameController.text = data[kDeceasedNameField] as String? ?? '';
    if (data[kDeceasedDodField] is Timestamp) {
      _deceasedDodController.text = _commonDateFormat
          .format((data[kDeceasedDodField] as Timestamp).toDate());
    }
    if (data[kDeceasedDobField] is Timestamp) {
      _deceasedDobController.text = _commonDateFormat
          .format((data[kDeceasedDobField] as Timestamp).toDate());
    }
    final positionData = data[kPositionField];
    if (positionData is GeoPoint) {
      _selectedGraveLocation =
          LatLng(positionData.latitude, positionData.longitude);
      _latLngController.text =
          "${_selectedGraveLocation!.latitude.toStringAsFixed(6)},${_selectedGraveLocation!.longitude.toStringAsFixed(6)}";
    }
    _selectedStateDropdown = data[kStateField] as String?;
    if (_selectedStateDropdown != null &&
        !_malaysianStates.contains(_selectedStateDropdown)) {
      _selectedStateDropdown = null;
    }
    _areaController.text = data[kAreaField] as String? ?? '';
    _selectedCategory = data[kCategoryField] as String?;
    if (_selectedCategory != null && !_categories.contains(_selectedCategory)) {
      _selectedCategory = null;
    }
    _graveLotController.text = data[kGraveLotField] as String? ?? '';
    _graveAddressController.text = data[kGraveAddressField] as String? ?? '';
    _cemeteryNameController.text = data[kCemeteryNameField] as String? ?? '';
    _relationshipToDeceasedController.text =
        data[kRelationshipToDeceasedField] as String? ?? '';
  }
  ...
  void dispose() {
    ...
    _deceasedDobController.dispose();
    _graveLotController.dispose();
    _graveAddressController.dispose();
    _cemeteryNameController.dispose();
    _relationshipToDeceasedController.dispose();
    super.dispose();
  }
  ...
  Timestamp? _parseDateToTimestamp(String? dateString) {
    ...
    try {
      final date = _commonDateFormat.parseStrict(dateString.trim());
  ...
  LatLng? _parseLatLngFromString(String latLngString) {
    try {
  ...
  Future<void> _selectDate(
    ...
    try {
      initialDate = _commonDateFormat.parseStrict(controller.text.trim());
    } catch (_) {
    ...
    if (picked != null) {
      controller.text = _commonDateFormat.format(picked);
    }
  }
  ...
  Future<void> _pickLocationOnMap() async {
    final LatLng? result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            GoogleMapPage(initialSelectedLocation: _selectedGraveLocation),
      ),
    );
    if (result != null) {
      setState(() {
        _selectedGraveLocation = result;
        _latLngController.text =
  ...
  bool _validateFormAndParseData() {
    if (_currentUser == null) {
      _showSnackbar('You must be logged in to submit a record.', isError: true);
      return false;
    }
    ...
    if (_selectedGraveLocation == null) {
      _showSnackbar(
          'Grave location coordinates are required. Please pin a location.',
          isError: true);
      return false;
    }
    return true;
  ...
  Future<void> _submitRecord() async {
    ...
    try {
      GeoPoint gravePositionForFirestore = GeoPoint(
          _selectedGraveLocation!.latitude, _selectedGraveLocation!.longitude);
      ...
      Map<String, dynamic> recordData = {
        ...
        kDeceasedDodField:
            _parseDateToTimestamp(_deceasedDodController.text.trim()),
        ...
        kDeceasedDobField:
            _parseDateToTimestamp(_deceasedDobController.text.trim()),
        ...
        kRelationshipToDeceasedField:
            _trimmedOrNull(_relationshipToDeceasedController),
      ...
      _showSnackbar('Failed to submit record. Please try again.',
          isError: true);
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final bool canSubmit =
        !_isProcessing && !_isLoadingUser && _currentUser != null;
        ...
        child: SafeArea(
          child: _isLoadingUser
              ...
              : _currentUser == null
                  ? Center(
                      child: Text(
                        "Error: User not loaded. Please log in again.",
                        style: TextStyle(
                          fontFamily: 'Metamorphous',
                        ...
                        child: ListView(
                          children: [
                            ...
                            _buildTextField(
                              ...
                              readOnly: true,
                              onTap: () =>
                                  _selectDate(context, _deceasedDodController),
                              validator: (v) => (v == null || v.trim().isEmpty)
                            ...
                            const SizedBox(height: 20),
                            _buildSectionTitle("Grave Lot Number",
                                isRequired: true),
                            _buildTextField(
                              controller: _graveLotController,
                              label: 'Grave Lot Number*',
                              validator: (v) => (v == null || v.trim().isEmpty)
                                  ? 'Grave Lot Number is required.'
                                  : null,
                            ),
                            const SizedBox(height: 20),
                            ...
                            _buildDropdown(
                              ...
                              value: _selectedStateDropdown,
                              label: 'State',
                              onChanged: (nv) =>
                                  setState(() => _selectedStateDropdown = nv),
                            ),
                            _buildTextField(
                            ...
                            _buildDropdown(
                              ...
                              label: 'Category',
                              onChanged: (nv) =>
                                  setState(() => _selectedCategory = nv),
                            ),
                            _buildTextField(
                              controller: _deceasedDobController,
                              label: 'Date of Birth',
                              hintText: 'DD/MM/YYYY',
                              readOnly: true,
                              onTap: () =>
                                  _selectDate(context, _deceasedDobController),
                            ),
                            _buildTextField(
                              controller: _relationshipToDeceasedController,
  ...
  Widget _buildLocationPickerTile() {
    String locationText = _latLngController.text.trim();
    bool hasSelectedLocation = _selectedGraveLocation != null;
    if (hasSelectedLocation && locationText.isEmpty) {
      locationText =
          "${_selectedGraveLocation!.latitude.toStringAsFixed(6)}, ${_selectedGraveLocation!.longitude.toStringAsFixed(6)}";
    }
    return ListTile(
      ...
      leading:
          Icon(Icons.map_outlined, color: AppColors.practiceCategory, size: 30),
      ...
      subtitle: Text(
        hasSelectedLocation
            ? 'Pinned: $locationText'
            : 'Tap to select or enter coordinates',
        style: TextStyle(
          fontFamily: 'Metamorphous',
          color: hasSelectedLocation
              ...
              : Colors.grey[700],
          fontSize: 13,
          fontWeight: hasSelectedLocation ? FontWeight.bold : FontWeight.normal,
        ),
        maxLines: 1,
            ...
            Builder(builder: (context) {
              return IconButton(
                ...
                onPressed: () async {
                  ...
                  if (box == null) return;
                  final coords = locationText;
                  if (coords.isEmpty) {
                    _showSnackbar("Location coordinates are not available.",
                        isError: true);
                    return;
                  }
                  final deceasedName = _deceasedNameController.text.trim();
                  final lotNumber = _graveLotController.text.trim();
                  String subjectText = deceasedName.isNotEmpty
                      ? "Grave Location for $deceasedName"
                      : "Grave Location";
                  String bodyText =
                      "Location for ${deceasedName.isNotEmpty ? deceasedName : 'the grave'}";
                  if (lotNumber.isNotEmpty) bodyText += " (Lot: $lotNumber)";
                  bodyText +=
                      ":\n\nhttps://www.google.com/maps/search/?api=1&query=$coords";

                  await Share.share(bodyText,
                      subject: subjectText,
                      sharePositionOrigin:
  ...
  Widget _buildSectionTitle(String title, {required bool isRequired}) {
    ...
    if (isRequired && !title.endsWith('*')) displayTitle += "*";
    return Padding(
  ...
  Widget _buildDropdown({
    ...
    String? currentValue =
        (items.isNotEmpty && value != null && items.contains(value))
            ? value
            : null;
    return Padding(
  ...
  InputDecoration _inputDecoration(String label, {String? hintText}) {
    return InputDecoration(
      ...
      focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12.0),
          borderSide:
              BorderSide(color: AppColors.practiceCategory, width: 1.5)),
  ...
  Widget _buildSubmitButton(String text, bool enabled) {
    return ElevatedButton(
      onPressed: enabled ? _submitRecord : null,
      style: ElevatedButton.styleFrom(
        padding: EdgeInsets.zero,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
        elevation: enabled ? 5 : 0,
        disabledBackgroundColor: Colors.grey.shade400.withOpacity(0.7),
      ...
      child: Ink(
        decoration: BoxDecoration(
          gradient: enabled
              ? LinearGradient(
                  colors: [AppColors.practiceCategory, Colors.green.shade500],
              ...
              : Row(
                  ...
                  children: [
                    ...
                    Text(
                      _isLoadingUser
                          ? "LOADING..."
                          : (_currentUser == null ? "LOGIN REQUIRED" : text),
                      style: const TextStyle(
                        fontFamily: 'Metamorphous',
                        fontSize: 16,
                        color: Colors.white,
  ...
  List<String> _generateSearchKeywords({
    ...
    deceasedName.split(RegExp(r'[\s/,-]+')).forEach(addKeyword);
    graveLot.split(RegExp(r'[\s/,-]+')).forEach(addKeyword);
    area?.split(RegExp(r'[\s/,-]+')).forEach(addKeyword);
    category?.split(RegExp(r'[\s/,-]+')).forEach(addKeyword);
